...
- `reminder.py` — the core orchestration. `build_reminder_targets(cfg, username_to_id)` fetches PRs per configured repo, applies the draft/stale-hours filters, computes `_reviewed_since_push()` per PR (an extra reviews + commits GitHub call), and calls `_collect_targets_for_pr()` per PR to decide who gets pinged, then merges results across repos into one `ReminderTarget` per Slack user (or per unmapped GitHub login).
- `github_client.py` — thin GitHub REST wrapper: pagination via `Link` headers, PR JSON -> `PullRequest` dataclass, `parse_body_mentions()` for `@login` extraction from PR bodies, and `fetch_pr_reviews()` / `fetch_pr_commits()` plus `latest_review_per_login()` / `latest_commit_at()` for the "already responded since the last push" check.
...
2. If empty and `reminders.ping_author_if_no_reviewer` is true -> the PR author.
...
Logins from sources 1 and 3 are further filtered against `reviewed_since_push`: a GitHub login whose most recent review/comment (`fetch_pr_reviews`) is at least as new as the PR's latest commit (`fetch_pr_commits`) is dropped, so someone who already approved/commented/requested-changes is not re-pinged until the author pushes a new commit. The author fallback (source 2) is exempt from this filter.
...
- Every non-draft, non-stale PR now costs two extra GitHub API calls (`fetch_pr_reviews`, `fetch_pr_commits`) on top of the paginated PR list, for the reviewed-since-push check. Worth remembering if a large repo set ever runs into rate limits.
